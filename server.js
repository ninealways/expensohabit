...
app.put('/api/credit-cards/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (req.body.currentCycleMonth !== undefined) updates.currentCycleMonth = normalizeMonthKey(req.body.currentCycleMonth) || existing.currentCycleMonth || localDate().slice(0, 7);
    ...
    if (req.body.outstanding !== undefined) updates.outstanding = Number(req.body.outstanding || 0);
    if (req.body.paid !== undefined) updates.paid = Number(req.body.paid || 0);
    ...
    for (const field of ['outstanding','paid','creditLimit','annualFee','waiverSpendLimit']) if (updates[field] !== undefined && updates[field] !== null && (!Number.isFinite(updates[field]) || updates[field] < 0)) return res.status(400).json({ error:'Amounts cannot be negative.' });
    if (updates.outstanding !== undefined || updates.paid !== undefined || updates.status !== undefined || updates.currentCycleMonth !== undefined) {
      const billMonth = updates.currentCycleMonth || existing.currentCycleMonth || localDate().slice(0, 7);
      const bills = Array.isArray(existing.bills) ? existing.bills.filter(bill => bill.month !== billMonth) : [];
      bills.push({ month:billMonth, outstanding:updates.outstanding ?? Number(existing.outstanding || 0), paid:updates.paid ?? Number(existing.paid || 0), status:updates.status || existing.status || 'Upcoming', updatedAt:new Date() });
      updates.bills = bills.sort((a, b) => String(b.month).localeCompare(String(a.month)));
    }
