...
function renderCalendarSummary(rows, selectedRows) {
  ...
  return `<aside class="calendar-sidebar">
    ...
    <div class="panel calendar-summary-card"><p class="panel-kicker">HIGHEST DAY</p><h3>${highest?.date || 'No spend yet'}</h3><strong class="calendar-focus-total">${highest ? money(highest.total) : money(0)}</strong><small>${highest ? `${highest.count} entries in selected range` : 'Add transactions to populate this card.'}</small></div>
